...
GSEA_Visualization_Figure3 <- function(DEG_Analysis_New, GSEA_Filtered_New, Batch_Corrected_Counts_New, ccbr699_metadata_for_NIDAP, msigdb_v6_2_with_orthologs) {
...
## PARAMETERS
    ...
    genescore_df <- DEG_Analysis_New
    gex_df <- Batch_Corrected_Counts_New
    metadata_df <- ccbr699_metadata_for_NIDAP
    gsea_df <- GSEA_Filtered_New
...
if (grepl("LE", which_plot)) {
    ...
    samples_to_include = setdiff(c("Sample_Roberts_41","Sample_Roberts_42","Sample_Roberts_43","Sample_Roberts_44","Sample_Roberts_53","Sample_Roberts_54","Sample_Roberts_55","Sample_Roberts_45","Sample_Roberts_46","Sample_Roberts_47","Sample_Roberts_48","Sample_Roberts_37","Sample_Roberts_39","Sample_Roberts_40"), geneid)
...
png(filename="GSEA_Visualization_Figure3.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
print("template_function_GSEA_Visualization_Figure3.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_DEG_Analysis_New<-readRDS(paste0(rds_output,"/var_DEG_Analysis_New.rds"))
Input_is_Seurat_count <- 0
for(item in var_DEG_Analysis_New){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_DEG_Analysis_New<-as.data.frame(var_DEG_Analysis_New)}else{var_DEG_Analysis_New <- var_DEG_Analysis_New}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_GSEA_Filtered_New<-readRDS(paste0(rds_output,"/var_GSEA_Filtered_New.rds"))
Input_is_Seurat_count <- 0
for(item in var_GSEA_Filtered_New){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_GSEA_Filtered_New<-as.data.frame(var_GSEA_Filtered_New)}else{var_GSEA_Filtered_New <- var_GSEA_Filtered_New}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_Batch_Corrected_Counts_New<-readRDS(paste0(rds_output,"/var_Batch_Corrected_Counts_New.rds"))
Input_is_Seurat_count <- 0
for(item in var_Batch_Corrected_Counts_New){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_Batch_Corrected_Counts_New<-as.data.frame(var_Batch_Corrected_Counts_New)}else{var_Batch_Corrected_Counts_New <- var_Batch_Corrected_Counts_New}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_ccbr699_metadata_for_NIDAP<-readRDS(paste0(rds_output,"/var_ccbr699_metadata_for_NIDAP.rds"))
Input_is_Seurat_count <- 0
for(item in var_ccbr699_metadata_for_NIDAP){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_ccbr699_metadata_for_NIDAP<-as.data.frame(var_ccbr699_metadata_for_NIDAP)}else{var_ccbr699_metadata_for_NIDAP <- var_ccbr699_metadata_for_NIDAP}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_GSEA_Visualization_Figure3<-GSEA_Visualization_Figure3(var_DEG_Analysis_New,var_GSEA_Filtered_New,var_Batch_Corrected_Counts_New,var_ccbr699_metadata_for_NIDAP,var_msigdb_v6_2_with_orthologs)
invisible(graphics.off())
saveRDS(var_GSEA_Visualization_Figure3, paste0(rds_output,"/var_GSEA_Visualization_Figure3.rds"))
