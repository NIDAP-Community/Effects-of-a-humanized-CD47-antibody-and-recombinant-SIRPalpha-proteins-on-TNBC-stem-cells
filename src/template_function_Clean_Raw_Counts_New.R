...
Clean_Raw_Counts_New <- function(counts_matrix) {
...
print("template_function_Clean_Raw_Counts_New.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_counts_matrix<-readRDS(paste0(rds_output,"/var_counts_matrix.rds"))
Input_is_Seurat_count <- 0
for(item in var_counts_matrix){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_counts_matrix<-as.data.frame(var_counts_matrix)}else{var_counts_matrix <- var_counts_matrix}
invisible(graphics.off())
var_Clean_Raw_Counts_New<-Clean_Raw_Counts_New(var_counts_matrix)
invisible(graphics.off())
saveRDS(var_Clean_Raw_Counts_New, paste0(rds_output,"/var_Clean_Raw_Counts_New.rds"))
