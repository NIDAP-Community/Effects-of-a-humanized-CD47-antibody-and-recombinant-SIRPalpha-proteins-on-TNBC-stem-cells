...
GSEA_Filtered_New <- function(GSEA_Preranked_New, msigdb_v6_2_with_orthologs ) {
...
print("template_function_GSEA_Filtered_New.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_GSEA_Preranked_New<-readRDS(paste0(rds_output,"/var_GSEA_Preranked_New.rds"))
Input_is_Seurat_count <- 0
for(item in var_GSEA_Preranked_New){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_GSEA_Preranked_New<-as.data.frame(var_GSEA_Preranked_New)}else{var_GSEA_Preranked_New <- var_GSEA_Preranked_New}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_GSEA_Filtered_New<-GSEA_Filtered_New(var_GSEA_Preranked_New,var_msigdb_v6_2_with_orthologs)
invisible(graphics.off())
saveRDS(var_GSEA_Filtered_New, paste0(rds_output,"/var_GSEA_Filtered_New.rds"))
