...
Filtered_Counts_New <- function(Clean_Raw_Counts_New, ccbr699_metadata_for_NIDAP) {
    ...
    #Basic Parameters:
    counts_matrix = Clean_Raw_Counts_New
    sample_metadata <- ccbr699_metadata_for_NIDAP   
    gene_names_column <- "Gene"
    columns_to_include <- c("Gene","Sample_Roberts_41","Sample_Roberts_42","Sample_Roberts_43","Sample_Roberts_44","Sample_Roberts_53","Sample_Roberts_54","Sample_Roberts_55","Sample_Roberts_56","Sample_Roberts_45","Sample_Roberts_46","Sample_Roberts_47","Sample_Roberts_48","Sample_Roberts_37","Sample_Roberts_39","Sample_Roberts_40")
    sample_names_column <- "Sample"
    groups_column <- "Group"
    labels_column <- "Label"

    #Filtering Parameters:
    outlier_samples_to_remove <- c("Sample_Roberts_56")
...
#print(colnames(df.final))
    ...
    png(
      filename="Filtered_Counts_New.png",
      width=imageWidth,
      height=imageHeight,
      units="px",
      pointsize=4,
...
print("template_function_Filtered_Counts_New.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_Clean_Raw_Counts_New<-readRDS(paste0(rds_output,"/var_Clean_Raw_Counts_New.rds"))
Input_is_Seurat_count <- 0
for(item in var_Clean_Raw_Counts_New){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_Clean_Raw_Counts_New<-as.data.frame(var_Clean_Raw_Counts_New)}else{var_Clean_Raw_Counts_New <- var_Clean_Raw_Counts_New}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_Filtered_Counts_New<-Filtered_Counts_New(var_Clean_Raw_Counts_New,var_ccbr699_metadata_for_NIDAP)
invisible(graphics.off())
saveRDS(var_Filtered_Counts_New, paste0(rds_output,"/var_Filtered_Counts_New.rds"))
